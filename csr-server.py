...
def send_message(x, type, message):
    ...
    connects[x]["connection"].send(f"{type}|{base64encode(message)}|{id}".encode("UTF-8"))
    return id
...
exit -- stop server''')
                ...
                for x in range(len(connects)):
                    print(f"{str(x)} | {connects[x]['address']} | {connects[x]['username']} | {connects[x]['status']}")
            elif i[0]=="disconnect":
...
def start_tcp_server():
    ...
    while True:
        c,addr=s.accept()
        connects.append({"connection":c,"address":addr,"username":"","status":True})
    ...
    def do_GET(self):
        ...
        if self.address_string() in trusted or not enable_password:
            if ireq[0:6] == "/index":
                ...
                v=ireq.replace("/send-api/","").split("/") # ['0','message','114514']
                try:
            ...
            if ireq[0:14] == "/password-api/" : # /password-api/123456
                ipassword=unquote(ireq.replace("/password-api/","").replace("\n","").replace("\r",""))
